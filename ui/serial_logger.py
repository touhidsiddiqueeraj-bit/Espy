...
class SerialLogger(QWidget):
    ...
    def _build_ui(self):
        ...
        self._send_btn.setToolTip("Send command to device (Enter)")
        ...
        # Ctrl+C button — sends 0x03 (ASCII ETX) to interrupt a running sketch.
        # This is the same as pressing Ctrl+C in the Arduino IDE serial monitor.
        self._ctrl_c_btn = QPushButton("Ctrl+C")
        self._ctrl_c_btn.setObjectName("secondary")
        self._ctrl_c_btn.setFixedHeight(34)
        self._ctrl_c_btn.setFixedWidth(70)
        self._ctrl_c_btn.setEnabled(False)
        self._ctrl_c_btn.setToolTip("Send Ctrl+C (0x03) to interrupt the running sketch")
        self._ctrl_c_btn.clicked.connect(self._send_ctrl_c)
        self._ctrl_c_btn.setStyleSheet(
            f"QPushButton {{ font-size: 12px; padding: 0 8px; border-radius: 8px; "
            f"background: transparent; border: 1px solid {C['border']}; "
            f"color: {C['text_muted']}; }}"
            f"QPushButton:hover {{ border-color: {C['accent']}; color: {C['accent']}; }}"
            f"QPushButton:disabled {{ color: {C['text_faint']}; border-color: {C['border']}; }}"
        )
        sl.addWidget(self._ctrl_c_btn)

        # Ctrl+L button — sends 0x0C (form feed) to clear the terminal on the device side
        self._clear_term_btn = QPushButton("Cls")
        self._clear_term_btn.setObjectName("secondary")
        self._clear_term_btn.setFixedHeight(34)
        self._clear_term_btn.setFixedWidth(50)
        self._clear_term_btn.setEnabled(False)
        self._clear_term_btn.setToolTip("Send form-feed (0x0C) to clear the device's terminal")
        self._clear_term_btn.clicked.connect(self._send_form_feed)
        self._clear_term_btn.setStyleSheet(
            f"QPushButton {{ font-size: 12px; padding: 0 8px; border-radius: 8px; "
            f"background: transparent; border: 1px solid {C['border']}; "
            f"color: {C['text_muted']}; }}"
            f"QPushButton:hover {{ border-color: {C['accent']}; color: {C['accent']}; }}"
        ...
        sl.addWidget(self._clear_term_btn)
    ...
    def _refresh_ports(self):
        try:
            ...
            from discovery.usb import USB_VENDORS_WHITELIST
            ...
            for i, p in enumerate(ports):
                ...
                if esp_idx < 0 and p.vid is not None and p.vid in USB_VENDORS_WHITELIST:
                    esp_idx = i
            if esp_idx >= 0:
                self._port_combo.setCurrentIndex(esp_idx)
            elif not ports:
    ...
    def _on_connected(self):
        ...
        self._ctrl_c_btn.setEnabled(True)
        self._clear_term_btn.setEnabled(True)
    ...
    def _send_ctrl_c(self):
        """Send 0x03 (ASCII ETX) — same as Ctrl+C in a terminal.

        Interrupts a running sketch the same way Ctrl+C does in the
        Arduino IDE serial monitor. Useful for stopping runaway loops
        without rebooting the device.
        """
        if self._worker.isRunning():
            self._worker.send("\x03")
            self._log_line(">>> [Ctrl+C]")

    def _send_form_feed(self):
        """Send 0x0C (form feed) — clears most serial terminals."""
        if self._worker.isRunning():
            self._worker.send("\x0c")
            self._log_line(">>> [Form feed]")

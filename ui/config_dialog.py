...
class ConfigDialog(QDialog):
    ...
    NETWORK_LIBS = {
        "WiFi", "WebServer", "HTTPClient", "WiFiClient", "WiFiUdp",
        "ESPmDNS", "AsyncTCP", "AsyncUDP", "ESPAsyncWebServer",
        "PubSubClient", "MQTT", "DNSServer", "ArduinoOTA", "NTPClient",
        "ESP32Ping",
    }
    ...
    def __init__(self, cfg: InoConfig, target_device: str = "", parent=None):
        ...
        self.cfg = cfg
        self._needs_wifi = bool(cfg.wifi_ssid) or any(
            lib.name in self.NETWORK_LIBS for lib in cfg.libraries
        )
    ...
    def _build_ui(self):
        ...
        self._flash_btn.setEnabled(not self._needs_wifi or bool(self.cfg.wifi_password))
        ...
        # Wi-Fi section — hidden entirely when sketch has no networking
        self._wifi_container = QWidget()
        wifi_inner = QVBoxLayout(self._wifi_container)
        wifi_inner.setContentsMargins(0, 0, 0, 0)

        self.ssid_edit = QLineEdit(self.cfg.wifi_ssid)
        self.ssid_edit.setPlaceholderText("Network name")
        wifi_inner.addLayout(_field_row("Wi-Fi:", self.ssid_edit))

        right_col.addWidget(self._wifi_container)
        self._wifi_container.setVisible(self._needs_wifi)

        # Password section — hidden entirely when sketch has no networking
        self._pass_container = QWidget()
        pass_inner = QVBoxLayout(self._pass_container)
        ...
        self._pass_hint.setVisible(self._needs_wifi and not bool(self.cfg.wifi_password))
        pass_inner.addWidget(self._pass_hint)

        right_col.addWidget(self._pass_container)
        self._pass_container.setVisible(self._needs_wifi)
    ...
    def _validate_password(self):
        ...
        self._flash_btn.setEnabled(not self._needs_wifi or has_pw)
        self._pass_hint.setVisible(self._needs_wifi and not has_pw)
        self._refresh_pass_style()

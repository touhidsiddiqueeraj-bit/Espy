 ...
 *   easyesp_data, data, nvs,     0x3C0000, 0x2000
 */
...
void clearCrashCount() {
  ...
  prefs.putUInt(KEY_CRASH_COUNT, 0);
  prefs.putUInt(KEY_CRASH_TIME,  0);
  prefs.putUInt(KEY_TOTAL_CRASHES, 0);
  prefs.end();
}
...
bool readPreProvisionedCredentials() {
  const esp_partition_t* part = esp_partition_find_first(
    ESP_PARTITION_TYPE_DATA, ESP_PARTITION_SUBTYPE_ANY, "easyesp_data");
  if (!part) return false;

  uint8_t buf[196];
  if (esp_partition_read(part, 64, buf, sizeof(buf)) != ESP_OK) return false;
  if (buf[0] != 'E' || buf[1] != 'C') return false;

  uint8_t ssidLen = buf[2];
  if (ssidLen == 0 || ssidLen > 64) return false;

  uint8_t passLen = buf[3 + ssidLen];
  uint8_t nameLen = buf[4 + ssidLen + passLen];

  wifiSSID   = String((char*)(buf + 3), ssidLen);
  wifiPass   = String((char*)(buf + 4 + ssidLen), passLen);
  deviceName = String((char*)(buf + 5 + ssidLen + passLen), nameLen);

  if (deviceName.length() == 0) deviceName = "Espy";

  saveCredentials(wifiSSID, wifiPass, deviceName);
  Serial.printf("[PreProv] Credentials loaded from partition (%s)\n",
                wifiSSID.c_str());
  return true;
}
...
bool connectWiFi() {
  ...
  WiFi.mode(WIFI_STA);
  WiFi.setSleep(false);
  WiFi.setTxPower(WIFI_POWER_11dBm);  // Reduce TX power to prevent brownout on ESP32-CAM
...
void setupOtaServer() {
  ...
  server.onNotFound([]() {
    ...
    // ── Health endpoint — telemetry for the device manager ──
    // Returns RSSI, uptime, free heap, chip model, last reboot reason,
    // current/next partition. Used by Espy's sidebar to show device
    // health at a glance and catch flaky devices before they fail.
    if (uri == "/espy/health" && server.method() == HTTP_GET) {
      const esp_partition_t* running = esp_ota_get_running_partition();
    ...
    // ── Rollback endpoint — revert to the previous OTA partition ──
    // ESP32 dual-partition OTA: after a successful flash, the new
    // partition boots but is "unvalidated". If the user calls this
    // endpoint, we mark the current partition invalid and reboot into
    // the previous one. Lifesaver when a flash produces a broken
    // firmware and you don't want to re-flash over Wi-Fi.
    if (uri == "/espy/rollback" && server.method() == HTTP_POST) {
      const esp_partition_t* running = esp_ota_get_running_partition();
      if (!running) {
        server.send(500, "application/json",
                    "{\"status\":\"error\",\"reason\":\"no_running_partition\"}");
        return;
      }
      // Mark current invalid and reboot. ESP-IDF picks the other
      // (hopefully valid) partition on next boot.
      esp_err_t err = esp_ota_mark_app_invalid_rollback_and_reboot();
      // If we get here, the rollback failed (rare — usually we've
      // already rebooted by now).
      char resp[256];
      snprintf(resp, sizeof(resp),
        "{\"status\":\"error\",\"reason\":\"rollback_failed\","
        "\"esp_err\":%d,\"running_partition\":\"%s\"}",
        (int)err,
        running ? running->label : "?"
      );
      server.send(500, "application/json", resp);
...
void setup() {
  WRITE_PERI_REG(RTC_CNTL_BROWN_OUT_REG, 0);  // Disable brownout detector for ESP32-CAM
  Serial.begin(115200);

...
_BOARD_FLASH_CACHE: dict[str, bool] = {}

def _has_flashsize_menu(fqbn: str) -> bool:
    if fqbn in _BOARD_FLASH_CACHE:
        return _BOARD_FLASH_CACHE[fqbn]
    parts = fqbn.split(":")
    if len(parts) < 3:
        return True
    base = Path.home() / ".arduino15" / "packages" / parts[0] / "hardware" / parts[1]
    if not base.exists():
        return True
    versions = sorted(base.iterdir()) if base.is_dir() else []
    if not versions:
        return True
    bt = versions[-1] / "boards.txt"
    if not bt.exists():
        return True
    prefix = f"{parts[2]}.menu.FlashSize."
    with open(bt) as f:
        _BOARD_FLASH_CACHE[fqbn] = any(line.startswith(prefix) for line in f)
    return _BOARD_FLASH_CACHE[fqbn]
...
class CompilerWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            # Persist binary so it survives the finally rmtree
            persistent = tempfile.NamedTemporaryFile(
                prefix="espy_", suffix=".bin", delete=False
            )
            shutil.copy2(bin_path, persistent.name)
            bin_path = persistent.name
